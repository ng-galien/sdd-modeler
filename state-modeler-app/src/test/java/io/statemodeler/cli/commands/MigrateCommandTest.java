 ...
 *
 * <p>
 * Note: Tests requiring actual LLM execution are disabled as they need:
 * - Ollama server running locally
 * These tests validate CLI argument parsing, repository integration, and error
 * handling.
 */
class MigrateCommandTest {
    ...
    // Do not capture stdout/stderr in tests; assert based on repository state and
    // exit codes
    ...
    void shouldFailWhenSourceSdrNotFound() {
        ...
        command.repositoryMixin = new RepositoryMixin();
        command.repositoryMixin.testRepository = repository;
        var cmd = new CommandLine(command);

        // When
        int exitCode = cmd.execute("nonexistent", "hash2", "--dialect", "postgres");
    ...
    void shouldFailWhenTargetSdrNotFound() {
        ...
        repository.save(sdr1, "test-model", "1.0.0").get();
        ...
        // When
        int exitCode = cmd.execute("hash1", "nonexistent", "--dialect", "postgres");
    ...
    void shouldRejectUnsupportedDialect() {
        ...
        repository.save(sdr1, "test-model", "1.0.0").get();
        repository.save(sdr2, "test-model", "2.0.0").get();
        ...
        // When
        int exitCode = cmd.execute("hash1", "hash2", "--dialect", "mysql");
    ...
    void shouldReuseExistingMigrationByDefault() {
        ...
        // When
        int exitCode = cmd.execute("hash1", "hash2", "--dialect", "postgres");
    ...
    void shouldFindSdrByNameAndVersion() {
        ...
        var sdr1 = createTestSdr("hash1", "CREATE TABLE test;");
        var sdr2 = createTestSdr("hash2", "CREATE TABLE test2;");
        repository.save(sdr1, "orders", "1.0.0").get();
        repository.save(sdr2, "orders", "2.0.0").get();
        ...
        // When
        int exitCode = cmd.execute("orders:1.0.0", "orders:2.0.0", "--dialect", "postgres");
    ...
    void shouldHandleInvalidNameVersionFormat() {
        ...
        repository.save(sdr, "model", "1.0.0").get();
        ...
        // When
        int exitCode = cmd.execute("model:", "hash1", "--dialect", "postgres");
    ...
    void shouldUseDefaultModelName() {
        ...
        // When/Then - verify command initializes (can't test full execution without
        // LLM)
        assertNotNull(cmd);
    ...
    void shouldRequireOpenAiApiKeyWhenProviderOpenAi() throws Exception {
        ...
        // Then - behavior depends on environment: if OPENAI_API_KEY env var is not set,
        // expect exception
    ...
    void shouldHandleForceFlag() {
        ...
        int exitCode = cmd.execute("hash1", "hash2", "--dialect", "postgres", "--force");
