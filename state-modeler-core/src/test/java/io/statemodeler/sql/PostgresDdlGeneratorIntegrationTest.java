...
class PostgresDdlGeneratorIntegrationTest {
        ...
        void shouldGenerateCompleteOrdersDdlFromYaml() throws Exception {
                ...
                assertTrue(result.isSuccess(),
                                "Failed to load model: " + (result.isFailure() ? result.getCause() : ""));
                ...
                assertTrue(ddl.contains(
                                "ALTER TABLE public_states.cancelled_source ADD CONSTRAINT cancelled_source_paid_fk"));

                // Verify CHECK constraint for OR transitions
                assertTrue(ddl.contains(
                                "ALTER TABLE public_states.cancelled_source ADD CONSTRAINT cancelled_source_check"));
        ...
        void shouldGenerateWithNullSchemaDefaultingToStates() throws Exception {
                ...
                var entity = new EntityDef("task", "tasks", idAttr, Map.of(), Map.of("active", initialState), Map.of(),
                                Map.of());
        ...
        void shouldGenerateWithPublicStateSchema() throws Exception {
                ...
                var entity = new EntityDef(
                                "project", "projects", idAttr, Map.of(), Map.of("active", initialState), Map.of(),
                                Map.of());
        ...
        void shouldGenerateWithEmptyStringSchemasDefaultingToStates() throws Exception {
                ...
                var entity = new EntityDef(
                                "resource", "resources", idAttr, Map.of(), Map.of("active", initialState), Map.of(),
                                Map.of());
        ...
        void shouldGenerateIndexesOnForeignKeys() throws Exception {
                ...
                var paidState = new StateDef(
                                ...
                                List.of(),
                                Map.of("payment_method",
                                                new AttributeDef("payment_method", "text", false, false, null, null)));
                ...
                // No index on order_paid.order_id alone - UNIQUE constraints create implicit
                // indexes
                assertFalse(ddl.contains(
                                "CREATE INDEX idx_order_paid_order_id ON public_states.order_paid (order_id);"));

                // Composite index on order_paid.(previous_pending_id, order_id) for composite
                // FK
                assertTrue(
                                ddl.contains(
                                                "CREATE INDEX idx_order_paid_previous_pending_id_order_id ON public_states.order_paid (previous_pending_id, order_id);"));

                // No index on order_pending.order_id alone - UNIQUE constraints create implicit
                // indexes
                assertFalse(ddl.contains(
                                "CREATE INDEX idx_order_pending_order_id ON public_states.order_pending (order_id);"));

                // Verify UNIQUE constraints are created on entity_id (prevents cyclic
                // transitions)
                assertTrue(ddl.contains(
                                "ALTER TABLE public_states.order_paid ADD CONSTRAINT order_paid_order_id_unique UNIQUE (order_id);"));
                assertTrue(
                                ddl.contains(
                                                "ALTER TABLE public_states.order_pending ADD CONSTRAINT order_pending_order_id_unique UNIQUE (order_id);"));

                // Verify UNIQUE composite constraints (id, entity_id) for composite FK targets
                assertTrue(
                                ddl.contains(
                                                "ALTER TABLE public_states.order_paid ADD CONSTRAINT order_paid_id_order_id_unique UNIQUE (id, order_id);"));
                assertTrue(
                                ddl.contains(
                                                "ALTER TABLE public_states.order_pending ADD CONSTRAINT order_pending_id_order_id_unique UNIQUE (id, order_id);"));
                assertTrue(
                                ddl.contains(
                                                "ALTER TABLE public_states.order_pending ADD CONSTRAINT order_pending_id_order_id_unique UNIQUE (id, order_id);"));
        }
        ...
        @Test
        void shouldGenerateDomainStateTableWithTriggers() throws Exception {
                // Given - simple order model
                var model = createSimpleOrderModel();
                var generator = DdlGenerators.forDialect("postgres");
                ...
                // Then - verify domain_state table creation
                assertTrue(ddl.contains("CREATE TABLE public_states.order_state"));
                assertTrue(ddl.contains("order_id INTEGER PRIMARY KEY NOT NULL")); // PK of entity
                assertTrue(ddl.contains("state_type TEXT NOT NULL"));
                assertTrue(ddl.contains("state_row_id BIGINT NOT NULL"));
                assertTrue(ddl.contains("state_at TIMESTAMPTZ NOT NULL"));
                assertTrue(ddl.contains("state_json JSONB NOT NULL"));

                // Verify FK to entity table
                assertTrue(
                                ddl.contains(
                                                "ALTER TABLE public_states.order_state ADD CONSTRAINT order_state_order_id_fk FOREIGN KEY (order_id) REFERENCES public.orders(id)"));
                ...
                // Verify trigger creation for pending state
                assertTrue(ddl.contains("CREATE TRIGGER order_pending_sync_order_state"));
                assertTrue(ddl.contains("AFTER INSERT ON public_states.order_pending"));
                assertTrue(ddl.contains("FOR EACH ROW"));
                assertTrue(ddl.contains("EXECUTE FUNCTION public_states.sync_order_state('PENDING')"));
        }
