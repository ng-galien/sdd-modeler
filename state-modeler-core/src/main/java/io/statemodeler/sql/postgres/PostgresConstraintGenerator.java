...
/**
 * Generates PostgreSQL constraint definitions for SDD entities, states, and extensions.
 * Handles UNIQUE constraints, foreign keys, and CHECK constraints for OR transitions.
 */
final class PostgresConstraintGenerator {
     ...
     * 2. UNIQUE(id, entity_id) - composite target for foreign keys (ensures aggregate integrity)
     *
     ...
     * Creates FKs to entity table and previous state tables (simple or OR transitions).
     * All FKs to state tables are composite: (column, entity_id) -> (id, entity_id).
     *
     * @param entity entity definition
     * @param state state definition
     ...
     * FKs to state tables are composite: (x_state_id, entity_id) -> (id, entity_id).
     *
     * @param entity entity definition
     * @param state state definition with OR transitions
     * @param entitySchema schema for entity tables
